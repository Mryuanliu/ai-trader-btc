...
import type { FuturesBacktestConfig } from './types';

/**
 * 合约回测 CLI（仅合约，现货回测已移除）。
 *
 * 用法：
 ...
 *     [--leverage=5] [--compare-leverage] [--stop-loss=0.05] [--take-profit=0.1]
 *
 * 数据不足时自动回填（Binance fapi 公共 REST，无需密钥）+ 资金费率回填。
...
async function runFuturesCli(input: {
  ...
  strategyName: string;
  exitRules: FuturesBacktestConfig['exitRules'];
