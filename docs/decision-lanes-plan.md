...
### 阶段 3：修复策略缺陷（在回测验证下进行）

**目标**：修复死区、归一化、量能三个缺陷。

**改动**：
- `trend-following.ts`：改分母为全权重、改量能为闭合 K 线、统一阈值口径

**验收**：
- 单测覆盖三个缺陷的具体用例
- **用回测对比修复前后的绩效曲线**，确认修复真的改善了结果而非只是「看起来更合理」
...
**改动**：
- 新增出场检查环节（在决策流程最前，优先级最高）
- 新增 `mean_reversion`、`breakout` 策略
- 策略参数持久化到 `strategyParams`

**验收**：
- 构造亏损持仓，验证止损优先于开仓信号触发
- 三个策略在同一行情下的回测绩效对比
