...
> 编写日期：2026-08-30 ｜ 状态：**A/B 期已完成并上线，C 期部分完成，D 期未开始**
>
> ## ⚡ 实施进度速览（2026-08-30 更新，接手先看这里）
>
> | 期 | 状态 | 关键结论 |
> |---|---|---|
> | **A 可观测** | ✅ 完成 | 阻塞原因码 + proximity + 信号归因 + 诊断面板 + 诊断 API 均已上线。**已实证价值**：靠它发现了"风控 100% 拦截被误判为信号不触发"的归因错位 |
> | **B 修缺陷** | ✅ 完成 | B3 RSI 语义参数化**已上线**（修掉语义反转）；B2 consensus 口径**已实现但默认不启用**（离散取值缺陷）；B1 阈值扫描工具已沉淀 |
> | **C 验信号** | 🟡 C1/C2 完成，**C3 暂停** | **重大发现：成本是第一约束**。IC 分析证实趋势信号方向系统性反了、唯一正 IC 是 bollinger（均值回归）；权重对照发现**毛收益为正、净收益为负**（@0.1% 费率）——问题不在信号在成本 |
> | **D 强能力** | ⏸️ 未开始 | **主动暂停**。净收益为负时加因子/加策略只会放大交易频率放大亏损，应先完成费率优化 |
>
> ## 🔴 最重要的发现：成本是生死线（C2 实证）
>
> ```
> 毛收益（feeRate=0）：   +2.31%  夏普 2.23   ← 信号有价值
> 净收益（0.1% 费率）：  -18.23%  夏普 -17.88 ← 被手续费磨光
> ```
> 线上 mean_reversion @1m 简化回测：胜率 69.3% 却净亏 -45.54%（238 次交易 × 0.3% 成本）。
>
> **费率敏感性（生死线 ≈ 单边 0.075%）**：
> | 单边费率 | 样本外净收益 | 判定 |
> |---|---|---|
> | 0.10%（标准） | -0.67% | ❌ |
> | **0.075%（BNB 抵扣）** | **+0.43%** | 转正 |
> | 0.05%（VIP） | +1.54% | ✅ 两窗均正 |
>
> ## 已应用的止血参数（2026-08-30，均已验证生效）
> - 现货：RSI 阈值 30/70 → **25/75**（最大改善项，毛收益 1.47%→3.84%）；`minOrderIntervalSec` 60 秒 → **1800 秒**（原配置理论一天 1440 单，是高频亏损主因）
> - 合约：`leverage` 5→3、`positionPct` 0.1→0.05（趋势信号 IC 为负 + 杠杆放大错误方向，降暴露）
> - 生效验证：RSI=53.4/bandPos=0.786 时接近度 0.712 = 新参数的理论值 ✅
>
> ## ⏭️ 后续待办（按优先级）
> 1. **【用户线下】开通 BNB 手续费抵扣或 VIP** —— 唯一能把策略从亏转盈的杠杆（-0.67% → +0.43%）
> 2. **观察 2~3 天**新参数下的真实成交与净盈亏（诊断面板 + orders 表）
> 3. 视观察结果决定：C3 防过拟合是否恢复 / D 期是否重启
> 4. 合约策略待定：趋势信号 IC 为负，`trend_following` 在均值回归行情下期望为负，考虑换 `mean_reversion` 或引入 regime 选择器（D1）
>
> 分析工具已沉淀（复用勿重写）：
> - `apps/server/scripts/signal-ic.ts`（IC/HAC 检验）
> - `apps/server/scripts/weight-ab.ts`（权重对照，注意排除 0 交易方案）
> - `apps/server/scripts/param-scan.ts`（参数网格 + 费率敏感性）
> - `apps/server/scripts/threshold-scan.ts`（阈值扫描）
> - `packages/shared/src/analysis/ic.ts`（统计函数，41 个单测）
>
> ⚠️ 回测脚本的两个教训（踩过）：①**必须排除 0 交易方案**（不交易≠好策略，曾把 0 交易的方案判为最优）；②**必须加持仓限频**（否则手续费吞掉本金，得出"所有方案都巨亏"的误导结论）。**决策依据是净收益，不是毛收益，更不是触发率。**

---

## 以下为原方案正文（含已完成项的原始设计，供追溯设计意图）
